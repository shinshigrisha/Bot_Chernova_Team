...
n8n используется как **оркестратор вокруг бэкенда**, а не вместо него: для уведомлений администраторам, ночных отчётов, экспортов и human-in-the-loop approval flow'ов.  
Исходником истины по пользователям и заявкам остаётся Python + Postgres.

- Для пилота «verification mirror» (зеркалирование заявки на верификацию в n8n) см. подробный гид: `docs/N8N_PILOT_VERIFICATION_MIRROR.md`.
- Для локального запуска n8n есть отдельный compose-файл: `infra/n8n/docker-compose.yml` и пример переменных окружения `infra/n8n/.env.n8n.example`:
