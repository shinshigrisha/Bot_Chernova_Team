...
**Основной обзор проекта — этот файл (README.md).** Детали по архитектуре, AI, админке, тестам и эксплуатации — в каноническом наборе в `docs/` (см. [Документация docs/](#документация-docs)).
...
3. [Документация docs/](#документация-docs)
...
9. [AI-куратор](#ai-куратор)
10. [Установка локальных эмбеддингов MiniLM](#установка-локальных-эмбеддингов-minilm)
11. [Команды бота](#команды-бота)
12. [Роли и админ-панель](#роли-и-админ-панель)
13. [Тесты](#тесты)
14. [Полезные команды](#полезные-команды)
15. [Развёртывание](#развёртывание)
16. [Health / Ops чеклист](#health--ops-чеклист)
17. [Устранение неполадок](#устранение-неполадок)
18. [Граница Python vs n8n](#граница-python-vs-n8n)
19. [Интеграция с n8n](#интеграция-с-n8n)
20. [Задачи и команды (Task Master)](#задачи-и-команды-task-master)
...
**Рантайм:** только `src/`. Каталог `app/` — legacy, в запуск бота не входит (подробнее: `docs/ARCHITECTURE_AUDIT_REPORT.md`).
...
│  AIFacade → AICourierService (RAG + LLM)    │
└─────────────────┬───────────────────────────┘
                  │
┌─────────────────▼───────────────────────────┐
│              Infrastructure                  │
...
**Поток AI-ответа:**
Единственная точка входа в AI — **AIFacade** (`src/core/services/ai/ai_facade.py`). Сообщение → AIFacade.answer_user/answer_admin → AICourierService (must_match → rules → FAQ → ML → ProviderRouter/LLM → fallback). Все вызовы LLM только через AIFacade/ProviderRouter; меню, доступ и верификация LLM не вызывают. Реализация: `src/core/services/ai/`; политика и промпты — `data/ai/`; FAQ — `src/infra/db/repositories/faq_repo.py`. Подробнее — [docs/ARCHITECTURE_AI_LAYER.md](docs/ARCHITECTURE_AI_LAYER.md).

### Документация (docs/)

| Документ | Назначение |
|----------|------------|
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | Слои приложения, сценарии, куда класть новую логику |
| [docs/ARCHITECTURE_AI_LAYER.md](docs/ARCHITECTURE_AI_LAYER.md) | AI: пайплайн, route classes, explainability, Curator vs Analyst |
| [docs/ADMIN_PANEL.md](docs/ADMIN_PANEL.md) | Админ-панель: пункты меню, доступ, верификация |
| [docs/TESTING.md](docs/TESTING.md) | Тесты: маркеры, команды, smoke |
| [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) | Развёртывание: сервисы, обновление, бэкапы |
| [docs/RUNBOOK.md](docs/RUNBOOK.md) | Ops: health-чеклист, устранение неполадок |

**Справочно (аудит/legacy):** [ARCHITECTURE_AUDIT_REPORT.md](docs/ARCHITECTURE_AUDIT_REPORT.md), [AI_LAYER_DUPLICATE_AUDIT.md](docs/AI_LAYER_DUPLICATE_AUDIT.md), [STABLE_AI_LAYER_REPORT.md](docs/STABLE_AI_LAYER_REPORT.md) — снимки аудита; актуальная архитектура в ARCHITECTURE.md и ARCHITECTURE_AI_LAYER.md. Каталог **app/** — legacy, не входит в рантайм ([app/README.md](app/README.md)).
...
| LLM | OpenAI-совместимый endpoint (конфиг: default / fast / reasoning / analytics / fallback) |
| Эмбеддинги | Локальный MiniLM (sentence-transformers/all-MiniLM-L6-v2), опционально OpenAI |
| HTTP-клиент | httpx, openai SDK |
...
LLM — через **OpenAI-совместимый** endpoint. Эмбеддинги — **локальный MiniLM** (внешний API не нужен).

| Переменная | Описание |
|---|---|
| `AI_ENABLED` | Включить AI (`true` / `false`, по умолчанию `false`) |
| `OPENAI_API_KEY` | API ключ для OpenAI-совместимого endpoint |
| `OPENAI_BASE_URL` | Базовый URL endpoint (например `https://api.example.com/v1`) |
| `AI_DEFAULT_MODEL` | Модель по умолчанию |
| `AI_FAST_MODEL` | Модель для быстрого диалога с курьерами |
| `AI_REASONING_MODEL` | Модель для сложных/неоднозначных кейсов |
| `AI_ANALYTICS_MODEL` | Модель для аналитики и отчётов |
| `AI_FALLBACK_MODEL` | Резервная (дешёвая) модель |
| `EMBEDDING_PROVIDER` | Эмбеддинги: `local` (MiniLM) или `openai` |
| `EMBEDDING_MODEL` | Модель эмбеддингов (при `local`: `sentence-transformers/all-MiniLM-L6-v2`) |

Подробнее: [Установка локальных эмбеддингов MiniLM](#установка-локальных-эмбеддингов-minilm).
...
│   │   ├── admin/
│   │   │   ├── admin_handlers.py      # /admin, главное меню админки
│   │   │   ├── admin_router.py        # Роутер админ-разделов
│   │   │   ├── admin_menu.py          # Сборка клавиатуры админ-меню
│   │   │   ├── ai_admin.py            # AI: /ai_status, /ai_policy_reload, FAQ
│   │   │   ├── incident.py            # Инциденты (журнал смены)
│   │   │   ├── ingest.py              # Импорт CSV
│   │   │   ├── tmc_issue.py           # Выдача ТМЦ
│   │   │   └── admin.py               # Хелперы доступа (is_admin_user и др.)
│   │   └── middlewares/
...
│   │               ├── openai_compatible_provider.py  # Основной: OpenAI-совместимый endpoint
│   │               ├── openai_provider.py              # OpenAI API
│   │               ├── groq_provider.py                # Groq (опционально)
│   │               └── deepseek_provider.py           # DeepSeek (опционально)
│   └── infra/
│       ├── db/
...
│   ├── rebuild_embeddings.py          # Пересборка эмбеддингов (FAQ + ML-кейсы)
│   ├── rebuild_faq_embeddings.py      # Только эмбеддинги FAQ
│   └── rebuild_case_embeddings.py     # Только эмбеддинги ml_cases
├── tests/
...
1. Rule-based (ключевые слова / must-match из core_policy.json)
       │ нет совпадения
       ▼
2. FAQ-поиск (семантический + ILIKE по таблице faq_ai)
       │ нет совпадения или низкий score
       ▼
3. LLM-генерация (OpenAI-совместимый endpoint: default / fast / reasoning / analytics / fallback по конфигу)
       │ нет ключей / ошибка
       ▼
...
# Эмбеддинги — локальный MiniLM (по умолчанию), внешний API не нужен:
...
## Установка локальных эмбеддингов MiniLM

Эмбеддинги для семантического поиска по FAQ и ML-кейсам по умолчанию строятся **локально** (без облачного API).

1. **Зависимости:** в проекте уже есть `sentence-transformers`. Для Docker-образа убедитесь, что в `Dockerfile` установлены зависимости для запуска модели (Python 3.11+, достаточно `pip install -e .` из `pyproject.toml`).
...
3. **Первый запуск:** при первом вызове эмбеддингов модель скачивается из Hugging Face (кэш в `~/.cache/huggingface/`). Для офлайн-среды предзагрузите модель на машине с интернетом, затем скопируйте кэш в контейнер или на сервер.

4. **Проверка:** запуск скрипта проверки эмбеддингов:
   ```bash
   python scripts/smoke_embeddings.py
   ```
   Ожидается вывод с `EMBEDDING_PROVIDER=local`, `EMBEDDING_MODEL=sentence-transformers/all-MiniLM-L6-v2` и размерностью вектора 384.

5. **Пересборка эмбеддингов FAQ/кейсов** (после изменения данных):
   ```bash
   python scripts/rebuild_faq_embeddings.py
   python scripts/rebuild_case_embeddings.py
   # или оба: python scripts/rebuild_embeddings.py
   ```
...
| AI не активен (/ai_status) | `AI_ENABLED=true`, `OPENAI_API_KEY` и `OPENAI_BASE_URL` для OpenAI-совместимого endpoint. |
