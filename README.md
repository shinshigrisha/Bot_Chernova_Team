...
**Поток AI-ответа:**
Единственная точка входа в AI — **AIFacade** (`src/core/services/ai/ai_facade.py`). Сообщение → AIFacade.answer_user/answer_admin → AICourierService (must_match → rules → FAQ → ML → ProviderRouter/LLM → fallback). Реализация: `src/core/services/ai/` (AIFacade, AICourierService, ProviderRouter, RAGService, providers), политика и промпты — `data/ai/`, FAQ — `src/infra/db/repositories/faq_repo.py`. Подробнее — `docs/ARCHITECTURE_AI_LAYER.md`.
