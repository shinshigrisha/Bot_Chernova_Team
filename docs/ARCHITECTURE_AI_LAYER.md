...
**Runtime:** только `src/`. Каталог `app/` не входит в рантайм (legacy; см. ARCHITECTURE_AUDIT_REPORT.md).
...
## Канонический путь (canonical AI path)

- **Точка входа:** AIFacade (answer_user, answer_admin, proactive_hint, analyze_csv).
- **Пайплайн куратора:** AICourierService (get_answer, get_risk_recommendation) → RAGService, IntentEngine, CaseEngine, CaseClassifier, FAQRepository; генерация — ProviderRouter.
- **Роутер LLM:** ProviderRouter (complete); провайдеры — только через роутер, без обхода в `src/`.
- **Retrieval:** RAGService.build_context; FAQRepository; CaseClassifier (ML-кейсы).
- **Отдельный контур аналитики:** AnalyticsAssistant (build_report), вызывается только через AIFacade.analyze_csv(); в пайплайн ответа курьеру не входит.

Явные классы маршрутов (route classes) в `ai_modes.py`: **no_llm**, **fast_chat**, **reasoning**, **analytics**, **fallback**. Все значения `AICourierResult.route` маппятся в один из этих классов через `ROUTE_TO_CLASS`; контур analytics использует класс `analytics` (отчёт не возвращает AICourierResult).
...
- Роутер провайдеров: `src/core/services/ai/provider_router.py`
- Аналитик (отдельный контур): `src/core/services/ai/analytics_assistant.py`

## Legacy / неиспользуемые в рантайме

- **app/** — не подключается к боту; при переносе функциональности вызовы LLM вести через AIFacade/ProviderRouter.
- **intent_ml_engine.py** — не импортируется; каноническая детекция интента: IntentEngine + IntentMLClassifier.
- **semantic_search.py** — не используется в пайплайне; канонический retrieval: RAGService + FAQRepository + get_embedding_service().
